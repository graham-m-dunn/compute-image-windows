...
<#
  .SYNOPSIS
    Activates a GCE instance in a GCVE private cloud (https://cloud.google.com/vmware-engine/docs/vmware-ecosystem/microsoft-licensing)
    This script takes the appropriate KMS server as a parameter.
    SPLAScript.ps1 -KMSServer 10.128.10.10 # by IP
    or
    SPLAScript.ps1 -KMSServer kms.windows.google.com # by hostname
#>
...
[void]$byolLicenses.Add('2089835370828997959') # windows-cloud/global/licenses/windows-10-enterprise-byol
[void]$byolLicenses.Add('8727879116868096918') # windows-cloud/global/licenses/windows-10-x64-byol
[void]$byolLicenses.Add('3732182829874353001') # windows-cloud/global/licenses/windows-10-x86-byol
[void]$byolLicenses.Add('5378533650449772437') # windows-cloud/global/licenses/windows-11-x64-byol
[void]$byolLicenses.Add('752112173778412950')  # windows-cloud/global/licenses/windows-7-enterprise-byol
[void]$byolLicenses.Add('5016528181960184510') # windows-cloud/global/licenses/windows-7-x64-byol
[void]$byolLicenses.Add('622639362407469665')  # windows-cloud/global/licenses/windows-7-x86-byol
[void]$byolLicenses.Add('7036859048284197429') # windows-cloud/global/licenses/windows-8-x64-byol
[void]$byolLicenses.Add('3720924436396315642') # windows-cloud/global/licenses/windows-8-x86-byol
[void]$byolLicenses.Add('5366577783322166007') # windows-cloud/global/licenses/windows-81-x64-byol
[void]$byolLicenses.Add('4551215591257167608') # windows-cloud/global/licenses/windows-server-2008-r2-byol
[void]$byolLicenses.Add('5559842820536817947') # windows-cloud/global/licenses/windows-server-2012-byol
[void]$byolLicenses.Add('6738952703547430631') # windows-cloud/global/licenses/windows-server-2012-r2-byol
[void]$byolLicenses.Add('4322823184804632846') # windows-cloud/global/licenses/windows-server-2016-byol
[void]$byolLicenses.Add('6532438499690676691') # windows-cloud/global/licenses/windows-server-2019-byol
[void]$byolLicenses.Add('2808834792899686364') # windows-cloud/global/licenses/windows-server-2022-byol

# Does the metadata server resolve and return the Google headers, i.e. are we likely in GCE? Use Test-Connection to avoid name not found exception 
if ((Test-Connection -ComputerName metadata.google.internal -Count 1 -ErrorAction SilentlyContinue) -And ((Invoke-WebRequest 'http://metadata.google.internal').Headers['Metadata-Flavor'] -eq 'Google')) {
    # Metadata server is reachable
    Write-Output "Metadata server is reachable. Checking for BYOL licenses..."
    ...
    Write-Output "Can't connect to metadata server. Continuing activation."
}
...
function Activate-Instance {
    ...
    .DESCRIPTION
      Tries to activate an nnstance with a KMS server. This function checks if the
      instance is already activated and if Yes skips. This function can be used
  ...
  #>
    ...
    if (Verify-ActivationStatus) {
        Write-Output "$script:hostname is already licensed and activated to $script:kms_server."
        return
    }

    Write-Output "$script:hostname needs to be activated by the $script:kms_server KMS Server."
    ...
    while ($retry_count -gt 0) {
        ...
        if (Verify-ActivationStatus) {
            Write-Output "Activation to $script:kms_server successful."
            break
        }
        else {
...
function Verify-ActivationStatus {
    <#
    .SYNOPSIS
      Check if the instance is activated to the desired KMS host.
    .OUTPUTS
      [bool] Is the instance activated to the desired KMS host.
  #>
    ...
    Write-Debug "Verifying Activation Status"

    try {
        $slmgr_status = & cscript //E:VBScript //nologo $env:windir\system32\slmgr.vbs /dli
    }
    catch {
        Write-Debug "Failed to get status"
        return $active
    }

    $status = $slmgr_status | Select-String -Pattern '^License Status:'
    $registered_kms_machine_name = Get-ItemPropertyValue -path 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\SoftwareProtectionPlatform' -name 'KeyManagementServiceName'

    # The initial space is to ensure "Unlicensed" does not match. 
    # If we are already activated against a different KMS server, remove the product key and continue activation. 
    Write-Debug "Status is $status"
    if ($status -match ' Licensed') {
        Write-Debug "Testing $script:kms_server against $registered_kms_machine_name"
        if (!($registered_kms_machine_name -eq $script:kms_server)) {
            Write-Debug "We are licensed but activated to $registered_kms_machine_name. Removing existing product key and continuing activation."
            $uninstall = & cscript //E:VBScript //nologo $env:windir\system32\slmgr.vbs /upk
            # Write-Debug "Returning $active"
            # return $active
        }
        else {
            $active = $true
